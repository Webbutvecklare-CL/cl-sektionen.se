	...
	const loadPost = async (oldPid) => {
		...
		getDoc(postRef)
				...
				if (docSnap.exists()) {
					...
					if (postData.type) {
						...
						if (postData.type === "event") {
							data.startDateTime = postData.startDateTime
								.toDate()
								.toLocaleDateString("sv");
							data.endDateTime = postData.endDateTime
								.toDate()
								.toLocaleDateString("sv");
						}
					}
					...
					setLoading(false);
					setError(
						"Inlägget fanns ej, kontrollera att du har angett en giltig länk.",
					);
					console.error(
			...
			.catch((err) => {
				setLoading(false);
				setError(
					"Det gick inte att ladda in inlägget. Kontakta webbansvariga.",
				);
	...
	const handleFormData = async (data) => {
		...
		for (const key in data) {
			...
			if (data[key] !== prefill[key]) {
				...
				if (
					key.includes("date") ||
					key.includes("startDateTime") ||
					key.includes("endDateTime")
				) {
		...
		if (data.image.name !== prefill.image.name) {
			...
			const oldImageRef = ref(
				storage,
				`posts/${postId}/${decodeURIComponent(prefill.image.name)}`,
			);
			try {
		...
		if (analytics) {
			logEvent(analytics, "post_update", {
				updated_keys: Object.keys(postData),
			});
		}
	};
			...
			{success && (
				<div>
					<p>
						...
						<Link
							href={`/aktuellt/${postId}`}
						>{`www.cl-sektionen.se/aktuellt/${postId}`}</Link>
						<br />
						<i>
							Observera att endast förtroendevalda som är inloggade kan se
							inlägget om det har ett senare publikationsdatum.
						</i>
						<br />
					</p>
				</div>
