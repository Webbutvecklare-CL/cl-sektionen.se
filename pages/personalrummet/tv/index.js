...
export default function Tv() {
	...
	const ImageHolder = ({ image }) => {
		...
		const fileName = decodeURIComponent(
			image.url.split("%2F")[1].split("?")[0],
		);

		const startDateComponents = image.startDate
			.toDate()
			.toLocaleDateString("sv-SE")
			.split("-");
		const endDateComponents = image.endDate
			.toDate()
			.toLocaleDateString("sv-SE")
			.split("-");
		...
		const EditMenu = () => {
			...
			const editHandler = async () => {
				...
				try {
					...
					setResponse(
						`Det gick inte att ändra·datumen! ${error.error.message}`,
					);
				}
				setEdit(false);
			};
					...
					<div>
						...
						<div className={styles.inputContainer}>
							...
							<input
								...
								min={
									new Date(new Date(newStartDate).getTime())
										.toISOString()
										.split("T")[0]
								}
								max={
									new Date(new Date(newStartDate).getTime() + sevenDays)
										.toISOString()
										.split("T")[0]
								}
							/>
		...
		return (
			<div className={styles.imageContainer}>
				...
				<div className={styles.imageOptions}>
					{(userData.uid === image.creator ||
						userData.permission === "admin") && <Options />}
					{!(
				...
				<div className={styles.imageHolder}>
					...
					{!response && !edit && (
						<Image src={image.url} alt={fileName} width={160} height={100} />
					)}
					{edit && <EditMenu />}
				</div>
			</div>
	...
	return (
		<div id="contentbody">
			...
			<p>
				Nedan kan du se vilka bilder som visas på TV:n och vilka samt vilka som
				har eller kommer att visas.
				<br />
				Du kan även ta bort och ändra bilder du själv laddat upp. När du
				uppdaterat behöver du ladda om sidan för att se uppdateringen. Bilderna
				sluta visas automatiskt, du behöver alltså inte ta bort bilden manuellt.
			</p>
			<button
				type="button"
				onClick={() => router.push("/personalrummet/tv/ladda-upp")}
			>
				Ladda upp bild
			</button>
			{error && <p>{error}</p>}
