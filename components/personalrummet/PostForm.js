...
export default function PostForm({ onSubmit, prefill, editMode = false }) {
	...
	); // Admins kan ändra vilken nämnd inlägget tillhör
	...
	const handleSubmit = (e) => {
		...
		const selectedTags = [];
		for (const pair of Object.entries(tags)) {
			if (pair[1]) {
	...
	const handleTagClick = (e) => {
		...
		if (type === "event") {
			...
			setTags((tags) => ({
				...tags,
				...{ SM: false, StyM: false, [tag]: !selected },
			}));
		} else {
					...
					<div>
                      ...
                      redigera det. Denna funktionen finns endast för dig som har admin-behörighet."
						...
						<div className={styles.visibilityInput}>
							...
							>
								<FontAwesomeIcon
									icon={visibility !== "public" ? faEyeSlash : faEye}
								/>
							</div>
							<p>
								{visibility === "public"
						...
						<input
							disabled={
								editMode || ((tags.StyM || tags.SM) && type === "event")
							}
							type="text"
							value={link}
